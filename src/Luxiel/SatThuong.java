...
//    }
    ...
    public static void damage(Player p, LivingEntity victim) {
        Bukkit.getScheduler().runTaskAsynchronously(Main.m, () -> {
            if (!Utils.damageable(victim, p)) return;
            ItemStack is = p.getInventory().getItemInMainHand();
            RPGPlayer rpgp = RPGPlayerListener.get(p);
            // item class check
            if (!ItemStats.getClasses(is).contains(rpgp.getPlayerClass())) {
                return;
            }
            // cooldown
            ...
            double itemst = ItemStats.DAMAGE.getStats(is);
            double playerstr = StatPlayer.sTr(p);
            double critchance = StatPlayer.chimang(p);
            double critdamage = StatPlayer.satthuongchimang(p);
            ...
            // GUILD BUFF FOR VICTIM
            if (victim instanceof Player) {
                Player zz = (Player) victim;
                enemydefense = StatPlayer.giap(zz);
                RPGPlayer rpgpp = RPGPlayerListener.get(zz);
                if (rpgpp.getGuild() != null) {
                    Guild g = rpgpp.getGuild();
            ...
            // crit hay 0
            if (!Utils.percentRoll(critchance)) {
                critdamage = 1;
            }
            // GUILD BUFF FOR ATTACKER
            if (rpgp.getGuild() != null) {
                Guild g = rpgp.getGuild();
                if (g.getFamilia() == Familia.AKUMA) victim.setFireTicks(40);
            ...
            // damage
            double totaldamage = CalcTotalDamage(itemst, playerstr, critdamage, othermodify, enemydefense);
            damageIndiCate(p, victim, totaldamage, critdamage >= 2, false);
            Bukkit.getScheduler().runTask(Main.m,()->{
            victim.damage(totaldamage + 0.1, p);
    ...
    public static void SpellDamage(Player p, LivingEntity victim, double damage) {
        Bukkit.getScheduler().runTaskAsynchronously(Main.m, () -> {
            ...
            ItemStack is = p.getInventory().getItemInMainHand();
            if(is == null) return;
            if(!ToolList.WEAPON.getItemList().contains(is.getType())) return;
            if (!ItemStats.getClasses(is).contains(rpgp.getPlayerClass())) {
                return;
            }
            ...
            Bukkit.getScheduler().runTask(Main.m, () -> {
                victim.damage(finalDamage + 0.1, p);
                victim.setNoDamageTicks(0);
            });
            damageIndiCate(p, victim, finaldamage, false, true);
        });
    }

    private static void damageIndiCate(LivingEntity p, LivingEntity e, double totaldamage, boolean crit, boolean spell) {
